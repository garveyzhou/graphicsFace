...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
  for(int i = 0; i < polygons ->lastcol;i+=3){
    double * norm = calculate_normal(polygons,i);
    if(norm[2] > 0){
...
void add_sphere( struct matrix * polygons,
  ...
  for ( lat = latStart; lat < latStop; lat++ ) {
    for ( longt = longStart; longt < longStop; longt++ ) {
      ...
      add_polygon( polygons, points->m[0][index1], points->m[1][index1] ,points->m[2][index1],
...
void add_torus( struct matrix * polygons,
  ...
  for ( lat = latStart; lat < latStop; lat++ ) {
    for ( longt = longStart; longt < longStop; longt++ ) {
      ...
      index0 = index0 % (points->lastcol);
      index1 = index1 % (points->lastcol);
      index2 = index2 % (points->lastcol);
      index3 = index3 % (points->lastcol);
      add_polygon(polygons,points->m[0][index0], points->m[1][index0], points->m[2][index0],
                           points->m[0][index1], points->m[1][index1], points->m[2][index1],
                           points->m[0][index3], points->m[1][index3], points->m[2][index3]);
      add_polygon(polygons,points->m[0][index0], points->m[1][index0], points->m[2][index0],
